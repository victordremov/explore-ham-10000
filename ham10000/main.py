...
from ham10000.type_bindings import ImageId, Path, TrainWithValidation, Holdout, MetaData
...
def main() -> None:
    metadata = preprocess_metadata(
        images_directory="./images", metadata_filename="HAM10000_metadata.csv"
    )
    ...
    model = initialize(models.resnet18(pretrained=True), n_classes=7, device=device)
    ...
    def log_training_results(trainer: Engine) -> None:
        ...
        print(
            f"\nTraining Results - Epoch: {trainer.state.epoch}  Avg accuracy: {metrics['accuracy']:.2f} Avg loss: {metrics['cross-entropy']:.2f}"
        )
    ...
    def get_evaluator_score(trainer: Engine) -> float:
        score = -evaluator.state.metrics['cross-entropy']
        return score


    early_stopping = EarlyStopping(patience=10, score_function=get_evaluator_score, trainer=trainer)
    evaluator.add_event_handler(Events.COMPLETED, handler=early_stopping)
